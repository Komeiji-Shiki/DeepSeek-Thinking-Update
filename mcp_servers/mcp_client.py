    ...
    def start(self) -> bool:
        ...
        try:
            ...
            for arg in args:
                ...
                # 优先按服务目录（cwd）解析
                if self.config.cwd:
                    cwd_path = os.path.join(self.config.cwd, arg)
                    if os.path.exists(cwd_path):
                        resolved_args.append(arg)
                        continue

                # 兼容旧配置：按当前工作目录（项目根）解析到绝对路径
            ...
            self.process = subprocess.Popen(
                [self.config.command] + resolved_args,
    ...
    def _read_stderr(self):
        ...
        while self.running and self.process:
            try:
                ...
                if line_str:
                    print(f"[{self.config.name} log] {line_str}")
                    
            except Exception as e:
                if self.running:
                    print(f"读取 MCP 错误输出错误: {e}")
                break
        ...
        try:
            ...
            for name, info in servers.items():
                ...
                if server_type == MCPServerType.STDIO:
                    # 处理args：如果有server_file就添加，否则直接使用配置的args
                    args = config.get("args", [])
                    if info.get("server_file"):
                        args = [info["server_file"]] + args
